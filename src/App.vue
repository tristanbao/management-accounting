...
function applyScene() {
  ...
  if (m === 'direct') {
    Object.assign(collectForm, {
      documentNo: 'FY-2026-0419-00' + (activeScene.value === 'customer' ? '86' : activeScene.value === 'product' ? '87' : activeScene.value === 'channel' ? '88' : '89'),
    ...
    Object.assign(collectForm, {
      documentNo: 'GS-2026-0419-00' + (activeScene.value === 'customer' ? '21' : activeScene.value === 'product' ? '22' : activeScene.value === 'channel' ? '23' : '24'),
      ...
      workHours: s.label === '客户经营活动' ? 48 : s.label === '产品销售推动' ? 36 : s.label === '渠道运营支撑' ? 24 : 16,
      workHourRatio: s.label === '客户经营活动' ? 32 : s.label === '产品销售推动' ? 25 : s.label === '渠道运营支撑' ? 18 : 10,
      benefitProducts: s.product,
      benefitOrgs: '九江银行南昌分行',
      enterPool: '直接归集',
      costSubject: s.costType,
      driverType: '工时占比',
      driverValue: s.label === '客户经营活动' ? 48 : s.label === '产品销售推动' ? 36 : s.label === '渠道运营支撑' ? 24 : 16,
      ratio: s.label === '客户经营活动' ? 32 : s.label === '产品销售推动' ? 25 : s.label === '渠道运营支撑' ? 18 : 10,
      dataSource: 'CRM系统',
      collectFreq: '月',
      poolLevel: '',
      poolRule: '',
      notes: `${s.label}服务关系归集，服务方：${s.manager}，受益对象：${s.customer}`
    })
  } else if (m === 'driver') {
    const driverData = {
      customer: { item: '高净值客户经营费', driver: '客户数', value: 328, ratio: 62, targets: [
        { name: 'A03财富客群（AUM≥600万）', ratio: '62%', value: '203户', amount: 177320 },
        { name: '普惠小微客群', ratio: '38%', value: '125户', amount: 108680 }
      ]},
      product: { item: '产品推广运营费', driver: '授信转化客户数', value: 286, ratio: 55, targets: [
        { name: '普惠经营贷', ratio: '55%', value: '157户', amount: 229900 },
        { name: '稳利盈6M理财', ratio: '45%', value: '129户', amount: 188100 }
      ]},
      channel: { item: '自助设备运营费', driver: '设备开机时长', value: 6000, ratio: 45, targets: [
        { name: '营业部网点', ratio: '38%', value: '2,280台时', amount: 253800 },
        { name: '零售支行', ratio: '34%', value: '2,040台时', amount: 226980 },
        { name: '社区银行', ratio: '28%', value: '1,680台时', amount: 187220 }
      ]},
      public: { item: '科技系统运维费', driver: '工时占比', value: 1200, ratio: 30, targets: [
        { name: '零售条线', ratio: '42%', value: '504工时', amount: 411600 },
        { name: '普惠条线', ratio: '35%', value: '420工时', amount: 343000 },
        { name: '对公条线', ratio: '23%', value: '276工时', amount: 225400 }
      ]}
    }
    const d = driverData[activeScene.value]
    Object.assign(collectForm, {
      documentNo: 'DF-2026-0419-00' + (activeScene.value === 'customer' ? '31' : activeScene.value === 'product' ? '32' : activeScene.value === 'channel' ? '33' : '34'),
      costSubject: d.item,
      driverType: d.driver,
      driverValue: d.value,
      ratio: d.ratio,
      ...
      dataSource: activeScene.value === 'channel' ? 'IT监控系统' : activeScene.value === 'product' ? 'CRM系统' : activeScene.value === 'public' ? '人力资源系统' : '核心业务系统',
      collectFreq: '月',
      amount: 0,
      org: '',
      line: '',
      ...
      poolLevel: '',
      poolRule: '',
      notes: `驱动因子：${d.driver}，受益对象：${s.customer}，驱动值：${d.value}`
    })
  } else {
    const poolData = {
    ...
    const p = poolData[activeScene.value]
    Object.assign(collectForm, {
      documentNo: 'CP-2026-0419-00' + (activeScene.value === 'customer' ? '41' : activeScene.value === 'product' ? '42' : activeScene.value === 'channel' ? '43' : '44'),
      costSubject: p.item,
      poolLevel: p.level,
      poolNature: p.nature,
      ...
      eventCode: '',
      owner: '',
      notes: `成本池层级：${p.level}，分摊维度：${p.dimension}，分摊规则：${p.ruleCode}，试算金额：${new Intl.NumberFormat('zh-CN', { style: 'currency', currency: 'CNY', maximumFractionDigits: 0 }).format(p.amount)}`
    })
  }
}
